...
### 1. Social Media Conversion Rate (Ad Effectiveness) ###
# Fixed value for social media users
total_social_media_users = 40  

# Count how many converted from Social Media
...
# *📊 Visualization - Social Media Conversion (3D Pie Chart)*
fig, ax = plt.subplots(figsize=(7, 6), subplot_kw={"projection": "rectilinear"})  # Normal 2D projection
...
ax.pie(
    ...
    startangle=140, shadow=True, explode=[0.1, 0], wedgeprops={"edgecolor": "black"}
)
ax.set_title("Social Media Conversion (3D Styled Pie Chart)")
plt.show()

### 2. Referral Program Effectiveness ###
df.rename(columns={"Numer of Referrals": "Number of Referrals"}, inplace=True)  # Fix column typo
...
# Visualization - Referrals Distribution (Histogram)
...
### 3. Who Recommended This? ###
referral_counts = df["Referral Source"].value_counts()

print("\nReferral Source Breakdown:\n", referral_counts)

# Visualization - Referral Sources (2D Bar Chart)
plt.figure(figsize=(7, 5))
